...
CREATE TABLE IF NOT EXISTS `customer_device` (
  ...
  `created` TIMESTAMP NOT NULL DEFAULT NOW() COMMENT 'time of device registration',
  ...
  `info` VARCHAR(255) NULL COMMENT 'additional custom data, JSON-encoded',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `token` (
  ...
  `amount` DECIMAL(13,4) NOT NULL COMMENT 'the amount in whole integer units, e.g. cents.',
  ...
  `expires` TIMESTAMP NULL COMMENT 'optional time of expiration',
  `refname` VARCHAR(36) NULL COMMENT 'a custom reference name, usually set be the token owner on creation',
  `info` VARCHAR(255) NULL COMMENT 'custom additional data, JSON-encoded. also holds the optional denomination data.',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `journal` (
  ...
  `data` TEXT NULL COMMENT 'JSON-encoded journal data',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `customer_param` (
  ...
  `pvalue` VARCHAR(1024) NULL COMMENT 'parameter value',
  `updated` TIMESTAMP NOT NULL DEFAULT NOW() COMMENT 'timestamp of last update or creation',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `clearing` (
  ...
  `debitor_account` VARCHAR(128) NOT NULL COMMENT 'debitor account information, arbitrary\nJSON data',
  `creditor_account` VARCHAR(128) NOT NULL COMMENT 'creditor account information, arbitrary\nJSON data',
  `status` TINYINT NOT NULL DEFAULT 0 COMMENT 'clearing status, updatable through clearing API',
  `created` TIMESTAMP NOT NULL DEFAULT NOW() COMMENT 'time of creation (token completion)',
  `updated` TIMESTAMP NULL COMMENT 'time of last modification',
  PRIMARY KEY (`id`),
